...
import { MERIDIAN_PATHWAY_CATALOG, evaluate, notYetOpenOn, pathwayById, statusOn } from '@meridian/pathways';
...
export async function generateMetadata({
  ...
  return { title: pathway === null ? 'Pathway' : pathway.name.en };
}
    ...
    <li className={styles.criterion}>
      <div className={styles.criterionHead}>
        ...
        <div className={styles.criterionBadges}>
          ...
          {result !== null ? (
            <Badge {...criterionStatusView(result.status)} />
          ) : (
            <Badge tone="neutral" label={bi('Not evaluated', 'No evaluado')} />
          )}
        </div>
      </div>
...
export default async function PathwayPage({
  ...
  const sample = SAMPLE_MATTERS.find((m) => m.matter.targetJurisdiction === pathway.jurisdiction) ?? null;
    ...
    <Page>
      ...
      {status === 'closed' && pathway.closureNote !== undefined ? (
        <Callout
          tone="bad"
          icon="✕"
          title={bi('Closed to new applications', 'Cerrada a nuevas solicitudes')}
        >
          <p className={styles.closureDate}>
            <TInline text={bi('Closed from', 'Cerrada desde el')} />{' '}
      ...
      {notYetOpenOn(pathway, AS_OF) ? (
        <Callout
          tone="info"
          icon="i"
          title={bi('Not yet open on the evaluation date', 'Aún no abierta en la fecha de evaluación')}
        >
          <TProse
            text={bi(
      ...
      <Section
        id="facts"
        title={bi('The record', 'El registro')}
        description={bi(
      ...
      >
        <Card tone="sunken">
          <Facts>
            <Fact label={bi('Catalog id', 'Identificador del catálogo')}>
              <code className={styles.inlineCode}>{pathway.id}</code>
            </Fact>
            <Fact label={bi('Rule version', 'Versión de la norma')}>
              <code className={styles.inlineCode}>{pathway.version}</code>
            </Fact>
            <Fact label={bi('First grant', 'Primera concesión')}>
              {pathway.durations.initialGrantMonths === undefined ? (
                <span className={styles.muted}>
                  <T text={bi('Not recorded', 'No consta')} />
                </span>
              ) : (
                <T
                  text={bi(
                    plural(pathway.durations.initialGrantMonths, 'month', 'months'),
                    `${pathway.durations.initialGrantMonths} meses`,
                  )}
                />
              )}
            </Fact>
            <Fact label={bi('Renewal', 'Renovación')}>
              {pathway.durations.renewalMonths === undefined ? (
                ...
                <T
                  text={bi(
                    ...
                    `${pathway.durations.renewalMonths} meses`,
                  )}
                />
              )}
            </Fact>
            <Fact label={bi('Counts toward naturalisation', 'Computa para la nacionalidad')}>
            ...
            <Fact label={bi('Published processing time', 'Plazo de tramitación publicado')}>
          ...
          {pathway.durations.note !== undefined ? (
            <TProse text={pathway.durations.note} className={styles.durationNote} />
          ) : null}

          {pathway.leadsTo.length > 0 ? (
            <div>
              <h3 className={styles.subheading}>
                <T text={bi('Leads to', 'Conduce a')} />
              </h3>
      ...
      <Section
        id="assessment"
        title={bi('Assessment against a worked example', 'Evaluación frente a un ejemplo resuelto')}
        description={
          sample === null
            ? bi(
          ...
          <Stack gap="md">
            <Card>
              <div className={styles.verdictHead}>
                <Badge {...verdictView(report.verdict)} />
                <span className={styles.verdictMeta}>
                  <TInline text={bi('Evaluated as at', 'Evaluado a fecha de')} />{' '}
                  <CivilDate value={report.asOf} />
                </span>
              </div>
              <Facts>
      ...
      >
        <ol className={styles.criteriaList}>
          {pathway.criteria.map((criterion) => (
            <CriterionRow key={criterion.id} criterion={criterion} report={report} />
          ))}
        </ol>
      </Section>

      <Section
        id="pathway-disclosure"
      ...
      >
        ...
        {unresolved.length > 0 ? (
          <ol className={styles.unresolvedList}>
            {unresolved.map((cid) => (
              <UnresolvedCitation key={cid} id={cid} />
            ))}
          </ol>
        ) : null}
      </Section>
