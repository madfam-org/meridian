...
export function NationalityTool({ locale }: { readonly locale: Locale }) {
  ...
  return (
    <Stack gap="lg">
      ...
      <Section
        id="nat-es-input"
        title={t('Your situation', 'Su situación')}
        description={t(
      ...
      >
        <PrivacyNote
          locale={locale}
          sourceHref={sourceUrl(SOURCE_PATH)}
          sourceLabel={SOURCE_PATH}
        />
        ...
        <Card tone="sunken">
          ...
          <form className={styles.form} onSubmit={onSubmit} noValidate>
            <fieldset className={styles.group}>
              <legend className={styles.legend}>{t('Nationality', 'Nacionalidad')}</legend>
              ...
              <SelectField
                ...
                error={t(issueFor(FIELD.claimed, issues))}
              ...
              {answers.claimed === OTHER_COUNTRY ? (
                <TextField
                  ...
                  label={t('Its two-letter country code', 'Su código de país de dos letras')}
                  hint={t(
                  ...
                  error={t(issueFor(FIELD.claimedOtherCode, issues))}
              ...
              <SelectField
                ...
                label={t('How you came to hold it', 'Cómo llegó a ostentarla')}
                hint={t(
                ...
                error={t(issueFor(FIELD.acquisition, issues))}
              ...
              <SelectField
                ...
                error={t(issueFor(FIELD.second, issues))}
              ...
              {answers.second === OTHER_COUNTRY ? (
                <TextField
                  ...
                  error={t(issueFor(FIELD.secondOtherCode, issues))}
              ...
              <SelectField
                ...
                error={t(issueFor(FIELD.residenceUnder, issues))}
            ...
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('Residence in Spain', 'Residencia en España')}
              </legend>
              ...
              <DateField
                ...
                error={t(issueFor(FIELD.residenceSince, issues))}
              ...
              <SelectField
                locale={locale}
                id={FIELD.status}
                label={t('Your situation in Spain today', 'Su situación en España a día de hoy')}
                hint={t(
                ...
                error={t(issueFor(FIELD.status, issues))}
            ...
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('Exams, language and conduct', 'Pruebas, idioma y conducta')}
              </legend>

              <NumberField
                locale={locale}
                id={FIELD.ageYears}
                label={t('Your age in completed years', 'Su edad en años cumplidos')}
                hint={t(
                ...
                error={t(issueFor(FIELD.ageYears, issues))}
              ...
              <SelectField
                ...
                error={t(issueFor(FIELD.ccse, issues))}
              ...
              <SelectField
                ...
                error={t(issueFor(FIELD.dele, issues))}
              ...
              <SelectField
                ...
                error={t(issueFor(FIELD.certificates, issues))}
            ...
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('The date this is measured at', 'La fecha a la que se mide')}
              </legend>

              <DateField
                locale={locale}
                id={FIELD.assessAsOf}
                label={t('Assess as at', 'Evaluar a fecha de')}
                hint={t(
                ...
                error={t(issueFor(FIELD.assessAsOf, issues))}
                required
            ...
            <ToolActions>
              <ToolButton
                type="submit"
                variant="primary"
                label={t('Measure my answers', 'Contrastar mis respuestas')}
              ...
              <ToolButton
                variant="clear"
                label={t('Clear everything', 'Borrar todo')}
                onClick={clearAll}
              />
            </ToolActions>
            ...
            <ToolActionGroup
              id="nat-es-examples"
              label={t('Or load an invented situation', 'O cargue una situación inventada')}
            >
      ...
      {assessment !== null ? (
        <NationalityResult locale={locale} assessment={assessment} focusKey={resultFocusKey} />
      ) : null}
    </Stack>
  );
}
...
function unknownsLabel(count: number): Bi {
  ...
  if (count === 1) {
    return bi(
      '1 criterion is not decided by your answers',
      '1 criterio no queda decidido por sus respuestas',
    );
  }
  return bi(
  ...
  return (
    <li className={cx(styles.criterion, styles[`is-${criterion.status}`])}>
      <div className={styles.criterionHead}>
        <h4 className={styles.criterionTitle}>{t(criterion.label)}</h4>
      ...
      {criterion.status === 'unknown' ? (
        <p className={styles.unknownNote}>{t(UNKNOWN_ROW_NOTE)}</p>
      ) : null}
      ...
      {criterion.evidence.length > 0 ? (
        <details className={styles.evidence}>
          <summary className={styles.evidenceSummary}>
            {t('The values the rule read', 'Los valores que leyó la norma')}
          </summary>
...
function RouteBlock({
  ...
  return (
    <ResultBlock
      id={`${RESULT_ID}-${pathway.id}`}
      title={t(pathway.name)}
      description={t(pathway.summary)}
    >
      <div className={styles.routeBadges}>
        <Badge tone={verdict.tone} label={t(verdict.label)} />
        <Badge tone={status.tone} label={t(status.label)} />
      ...
      <ol className={styles.criteria}>
        {route.criteria.map((criterion) => (
          <CriterionRow locale={locale} key={criterion.id} criterion={criterion} />
        ))}
      </ol>
    </ResultBlock>
  );
 ...
 */
function NoteCallout({ note, locale }: { readonly note: NoteView; readonly locale: Locale }) {
  const t = translator(locale);
  return (
    <Callout tone="warn" icon="!" title={t(NOTE_TITLE[note.code])}>
      {note.citationId !== undefined ? (
        <p className={styles.noteSource}>
          <span className={styles.noteSourceLabel}>{t('The source', 'La fuente')}</span>{' '}
      ...
      {note.criteria.length > 0 ? (
        <div className={styles.noteAffects}>
          <p className={styles.noteAffectsLabel}>{t('It affects', 'Afecta a')}</p>
          <ul className={styles.noteCriteria}>
            {note.criteria.map((label) => (
              <li key={label.en}>{t(label)}</li>
            ))}
          </ul>
        </div>
      ) : null}
...
function NationalityResult({
  ...
  return (
    <ResultPanel
      ...
      verdict={t(unknownsLabel(assessment.unknownCount))}
      tone={assessment.unknownCount > 0 ? 'warn' : 'info'}
      lead={t(PANEL_LEAD)}
      focusKey={focusKey}
    >
      <p className={styles.asOfLine}>
        {t('Measured as at', 'Medido a fecha de')} <CivilDate value={assessment.asOf} />
      </p>
      ...
      {/*
        ...
        not a correction.
      */}
      <CoverageResultNotice locale={locale} jurisdictions={['ES']} />

      <Callout tone="warn" icon="!" title={t(NOTE_TITLE.unreviewed_rule)}>
        {unreviewed !== undefined ? (
          <p className={styles.noteText} lang="en">
            {unreviewed.text}
          </p>
        ) : null}
        <p>{t(UNREVIEWED_CAVEAT)}</p>
      </Callout>
      ...
      >
        <p>{t(UNKNOWN_CAVEAT)}</p>
      </Callout>

      {reduced !== undefined ? <RouteBlock route={reduced} locale={locale} /> : null}
      ...
      {general !== undefined ? <RouteBlock route={general} locale={locale} /> : null}
      ...
      {assessment.residenceDurationDecided ? (
        ...
        >
          <p>{t(CONTINUITY_CAVEAT)}</p>
        </Callout>
      ) : null}

      {otherNotes.map((note) => (
        <NoteCallout locale={locale} key={note.key} note={note} />
      ))}

      <ResultBlock
        id={`${RESULT_ID}-sources`}
        title={t('The rules this applied', 'Las normas aplicadas')}
        description={t(
      ...
      >
        <CitationList locale={locale} citations={assessment.citations} asOf={assessment.asOf} />
      </ResultBlock>
