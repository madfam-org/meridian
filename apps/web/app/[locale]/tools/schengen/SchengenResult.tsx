...
function outcomeView(outcome: AllowanceOutcome): OutcomeView {
  switch (outcome) {
    case 'within':
      return {
        tone: 'ok',
        verdict: bi('Within the allowance', 'Dentro de la franquicia'),
      };
    case 'over':
      return {
        tone: 'bad',
        verdict: bi('Over the allowance', 'Por encima de la franquicia'),
      };
    case 'undetermined':
      return {
        tone: 'review',
...
function windowWorking(report: SchengenReport, locale: Locale): WorkingRow[] {
  const status = report.status.value;
  const rows: WorkingRow[] = [
    {
      label: bi('Allowance in any 180-day period', 'Franquicia en cualquier periodo de 180 días'),
  ...
  for (const range of status.countedRanges) {
    rows.push({
      ...
      op: '+',
      value: days(rangeLengthDays(range), locale),
    });
  }
  ...
  rows.push({
    ...
    op: '=',
    value: days(status.daysUsed, locale),
    note: bi(
  ...
  if (report.daysUsedIfAmbiguousCounted !== status.daysUsed) {
    rows.push({
      ...
      op: '=',
      value: days(report.daysUsedIfAmbiguousCounted, locale),
      note: bi(
...
function StayRow({ stay, locale }: { readonly stay: StayAnalysis; readonly locale: Locale }) {
    ...
    <tr>
      ...
      <td>
        {stay.countedRanges.length === 0 ? (
          <span className={styles.muted}>{t('Nothing charged', 'Nada imputado')}</span>
        ) : (
      ...
      <td>
        {stay.uncounted.length === 0 ? (
          <span className={styles.muted}>{t('Every day charged', 'Todos los días imputados')}</span>
        ) : (
          <ul className={styles.why}>
            {stay.uncounted.map((reason) => (
              <li key={reason.key}>
                <span className={styles.whyDays}>{days(reason.days, locale)}</span> {t(reason.text)}
              </li>
            ))}
          </ul>
        )}
...
export function SchengenResult({
  ...
  return (
    <ResultPanel
      ...
      title={t(
        'Where this record sits against the 90 days',
        'Qué dice este registro frente a los 90 días',
      )}
      verdict={t(view.verdict)}
      tone={view.tone}
      lead={t(outcomeLead(report))}
      focusKey={focusKey}
    >
            ...
            {report.conflicts.map((conflict, index) => (
              <li key={`${conflict.range.start}-${index}`}>
                <div className={styles.periodHead}>
                  ...
                  <span className={styles.numeric}>{days(conflict.days, locale)}</span>
            ...
            {report.ambiguous.map((period) => (
              <li className={styles.period} key={period.key}>
                <div className={styles.periodHead}>
                  ...
                  {t(period.countryName)}
                  <RangeText range={period.range} />
                  <span className={styles.numeric}>{days(period.days, locale)}</span>
                </div>
      ...
      <ResultBlock
        id="schengen-window"
        title={t('The window, and what is in it', 'La ventana y lo que contiene')}
        description={t(
      ...
      >
        <div className={styles.headline}>
          <Figure
            ...
            unit={t(
              `of ${SCHENGEN_MAX_DAYS} days charged`,
              `de ${SCHENGEN_MAX_DAYS} días imputados`,
            )}
            tone="strong"
          />
          <Badge tone={view.tone} label={t(view.verdict)} />
        </div>
        ...
        <Meter
          ...
          tone={
            report.outcome === 'within' ? 'accent' : report.outcome === 'over' ? 'bad' : 'review'
          }
        />

        <Facts>
          <Fact label={t('Window measured', 'Ventana medida')}>
            <RangeText range={report.window} />
          </Fact>
          <Fact label={t('Days charged', 'Días imputados')}>
            <Figure value={status.daysUsed} unit={t('days', 'días')} />
          </Fact>
          <Fact label={t('Days still available', 'Días aún disponibles')}>
            <Figure value={status.daysRemaining} unit={t('days', 'días')} />
          </Fact>
          <Fact label={t('Days beyond the limit', 'Días por encima del límite')}>
            <Figure value={status.daysOverLimit} unit={t('days', 'días')} />
          </Fact>
        </Facts>
        ...
        <Working locale={locale} rows={windowWorking(report, locale)} />
      </ResultBlock>

      <ResultBlock
        id="schengen-stays"
        title={t('Which stays charged which days', 'Qué estancias imputaron qué días')}
        description={t(
      ...
      >
        <ScrollX>
          <table>
            ...
            <thead>
              <tr>
                <th scope="col">{t('State', 'Estado')}</th>
                <th scope="col">{t('Stay as entered', 'Estancia introducida')}</th>
                <th scope="col">{t('Days', 'Días')}</th>
                <th scope="col">{t('Charged period', 'Periodo imputado')}</th>
                <th scope="col">{t('Days charged', 'Días imputados')}</th>
                <th scope="col">{t('Where the rest went', 'Adónde fue el resto')}</th>
              </tr>
            </thead>
            <tbody>
              {report.stays.map((stay) => (
                <StayRow locale={locale} key={stay.id} stay={stay} />
              ))}
            </tbody>
          </table>
        </ScrollX>
      ...
      {report.plannedTrip !== null ? (
        <ResultBlock
          ...
          title={t('The worst day of the range you gave', 'El peor día del intervalo indicado')}
          description={t(
        ...
        >
          <Facts>
            <Fact label={t('Range scanned', 'Intervalo analizado')}>
            ...
            <Fact label={t('Highest usage falls on', 'El consumo más alto cae el')}>
              <CivilDate value={report.plannedTrip.assessment.value.date} />
            </Fact>
            <Fact label={t('Days charged on that day', 'Días imputados ese día')}>
              <Figure
                value={report.plannedTrip.assessment.value.status.daysUsed}
                unit={t(`of ${SCHENGEN_MAX_DAYS}`, `de ${SCHENGEN_MAX_DAYS}`)}
                tone="strong"
              />
              <div className={styles.footnote}>
                {t(
            ...
            <Fact label={t('Days charged on the last day', 'Días imputados el último día')}>
              <Figure
          ...
          <Working
            ...
            rows={[
              ...
              },
              {
                ...
                op: '−',
                value: days(report.plannedTrip.assessment.value.status.daysUsed, locale),
              },
              {
                ...
                value: report.plannedTrip.assessment.value.status.compliant
                  ? days(report.plannedTrip.assessment.value.status.daysRemaining, locale)
                  : days(report.plannedTrip.assessment.value.status.daysOverLimit, locale),
                emphasis: true,
            ...
            >
              ...
              <ul className={styles.periodList}>
                {report.plannedTrip.uncovered.map((range) => (
                  <li key={`${range.start}-${range.end}`}>
                    <RangeText range={range} />{' '}
                    <span className={styles.numeric}>{days(rangeLengthDays(range), locale)}</span>
                  </li>
                ))}
              </ul>
            </Callout>
            ...
            <>
              <div className={styles.answer}>
                <Badge
                  ...
                  label={t('No such date', 'No hay tal fecha')}
                />
              </div>
              <p>
                {t(
                ...
                rows={[
                  {
                    ...
                    value: days(report.proposedStay.stayDays, locale),
                    note: bi(
                  ...
                  },
                  {
                    ...
                    value: days(report.proposedStay.horizonDays, locale),
                  },
                  {
      ...
      <ResultBlock
        ...
        title={t('The accession dates this used', 'Las fechas de adhesión utilizadas')}
        description={t(
      ...
      >
        <ScrollX>
          <table>
            <thead>
              <tr>
                <th scope="col">{t('State', 'Estado')}</th>
                <th scope="col">{t('Counted from', 'Computa desde')}</th>
                <th scope="col">{t('Staged from', 'Fase previa desde')}</th>
              </tr>
            </thead>
            <tbody>
              {statesUsed.map((code) => {
                ...
                return (
                  <tr key={code}>
                    <td>
                      <Chip>{code}</Chip> {state === null ? code : t(state.name)}
                    </td>
        ...
        <Callout
          tone="warn"
          icon="!"
          title={t('These dates are data, not a citation', 'Estas fechas son datos, no una cita')}
        >
          <p>
            {t(
      ...
      <ResultBlock
        id="schengen-sources"
        title={t('The rule this applied', 'La norma aplicada')}
        description={t(
      ...
      >
        <CitationList
          locale={locale}
          citations={[SCHENGEN_CITATION]}
          asOf={DEFAULT_REFERENCE_DATE}
        />
      </ResultBlock>
