...
export function SchengenTool({ locale }: { readonly locale: Locale }) {
  ...
  function addStay(): void {
    ...
    setAnswers((previous) => ({
      ...previous,
      stays: [...previous.stays, blankStayRow(key)],
    }));
  ...
  return (
    <Stack gap="lg">
      ...
      <Section
        id="schengen-input"
        title={t('Your stays', 'Sus estancias')}
        description={t(
          ...
          <form className={styles.form} onSubmit={onSubmit} noValidate>
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('Where you were, and when', 'Dónde estuvo y cuándo')}
              </legend>

              <p className={styles.groupHint}>{t(ENDPOINTS_NOTE)}</p>
                    ...
                    <li className={styles.stayItem} key={row.key}>
                      <fieldset className={styles.stayFields}>
                        <legend className={styles.stayLegend}>
                          {t(`Stay ${index + 1}`, `Estancia ${index + 1}`)}
                        </legend>
                        ...
                        <FieldRow>
                          <SelectField
                            ...
                            error={t(issueFor(countryId, issues))}
                            required
                          ...
                          <DateField
                            locale={locale}
                            id={startId}
                            label={t('Day you entered', 'Día de entrada')}
                            error={t(issueFor(startId, issues))}
                            required
                            value={row.start}
                        ...
                        <ToolActions>
                          <ToolButton
                            variant="clear"
                            label={t(`Remove stay ${index + 1}`, `Quitar la estancia ${index + 1}`)}
              ...
              <div className={styles.stayFooter}>
                <ToolActions>
                  <ToolButton
                    label={t('Add a stay', 'Añadir una estancia')}
                    onClick={addStay}
            ...
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('The day to measure', 'El día que se mide')}
              </legend>
              ...
              <DateField
                ...
                error={t(issueFor(FIELD.reference, issues))}
                required
            ...
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('A range to scan for its worst day', 'Un intervalo del que hallar su peor día')}
              </legend>
              ...
              <FieldRow>
                <DateField
                  ...
                  label={t('First day of the range', 'Primer día del intervalo')}
                  error={t(issueFor(FIELD.plannedStart, issues))}
                ...
                <DateField
                  locale={locale}
                  id={FIELD.plannedEnd}
                  label={t('Last day of the range', 'Último día del intervalo')}
                  error={t(issueFor(FIELD.plannedEnd, issues))}
            ...
            <fieldset className={styles.group}>
              <legend className={styles.legend}>
                {t('A stay you are considering', 'Una estancia que está considerando')}
              </legend>
              ...
              <FieldRow>
                <NumberField
                  ...
                  label={t('Length of the stay, in days', 'Duración de la estancia, en días')}
                  hint={t(
                  ...
                  error={t(issueFor(FIELD.proposedDays, issues))}
                ...
                <DateField
                  ...
                  label={t('Not starting before', 'Sin empezar antes del')}
                  error={t(issueFor(FIELD.proposedFrom, issues))}
            ...
            <ToolActions>
              <ToolButton
                type="submit"
                variant="primary"
                label={t('Count the days', 'Contar los días')}
            ...
            <ToolActionGroup
              ...
              label={t('Or load an invented itinerary', 'O cargue un itinerario inventado')}
            >
            ...
            <ul className={styles.exampleList}>
              {SCHENGEN_EXAMPLES.map((example) => (
                <li key={example.id}>
                  <span className={styles.exampleName}>{t(example.label)}</span> {t(example.note)}
                </li>
              ))}
            </ul>
          </form>
      ...
      {failure !== null ? (
        <Callout
          tone="bad"
          icon="✕"
          title={t('The day counter refused this input', 'El cómputo rechazó esta entrada')}
        >
      ...
      {report !== null ? (
        <SchengenResult report={report} focusKey={resultFocusKey} locale={locale} />
      ) : null}
    </Stack>
  );
}
