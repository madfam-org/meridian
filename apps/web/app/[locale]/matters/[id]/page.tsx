...
export async function generateMetadata({
  ...
  // No `alternates`: `/matters` and everything under it is disallowed in
  // `robots.ts` because a person's own file is not indexable material, and
  // declaring hreflang for a page a crawler is asked not to fetch is a
  // contradiction rather than a hint.
  return {
    title: sample === null ? t('Matter', 'Expediente') : t(sample.name),
  };
}
 ...
 */
export default async function MatterPage({ params }: { readonly params: Promise<MatterParams> }) {
  const locale = await readLocale(params);
  const t = translator(locale);
  const { id } = await params;
  const sample = sampleMatterById(id);
  ...
  return (
    <>
      <Section
        id="representation"
        title={t('Who is accountable', 'Quién responde')}
        description={t(
        ...
        <Stack gap="md">
          {sample.matter.representativeId === null ? (
            ...
            <Callout
              tone="ok"
              icon="§"
              title={t('A representative is assigned', 'Hay un representante asignado')}
            >
              <p>
                {t(
          ...
          {withheld ? (
            <Card tone="sunken">
              <h3 className={styles.subheading}>{t('What you keep', 'Qué conserva')}</h3>
      ...
      <Section
        id="route"
        title={t('The route being pursued', 'La vía que se sigue')}
        description={t(
          ...
          <Card>
            <div className={styles.routeHead}>
              <h3 className={styles.subheading}>
                <Link href={localizedPath(`/pathways/${pathway.id}`, locale)}>
                  {t(pathway.name)}
                </Link>
              </h3>
              <div className={styles.routeBadges}>
                ...
                <Badge
                  tone={reviewStatusView(pathway.reviewStatus).tone}
                  label={t(reviewStatusView(pathway.reviewStatus).label)}
                />
              </div>
            </div>
            <p>{t(pathway.summary)}</p>
          </Card>
        )}
            ...
            return (
              <Card key={report.pathwayId}>
                <h4 className={styles.assessmentTitle}>
                  <Link href={localizedPath(`/pathways/${report.pathwayId}`, locale)}>
                    {target === null ? report.pathwayId : t(target.name)}
                  </Link>
                </h4>
                ...
                <p className={styles.assessmentMeta}>
                  {t(
                    `${report.criteria.length} criteria · ${report.blockingFailures.length} blocking unmet · ${report.unknowns.length} not recorded`,
                    `${report.criteria.length} criterios · ${report.blockingFailures.length} bloqueantes no cumplidos · ${report.unknowns.length} sin datos`,
                  )}
                </p>
              </Card>
            );
      ...
      <Section
        id="phases"
        title={t('Where the matter stands', 'En qué punto está el expediente')}
        description={t(
      ...
      >
        <PhaseTimeline locale={locale} phases={view.phases} />
      </Section>
      ...
      <Section
        id="sources"
        title={t('Sources behind these tasks', 'Fuentes de estas tareas')}
        description={t(
