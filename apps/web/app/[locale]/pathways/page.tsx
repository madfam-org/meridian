...
export async function generateMetadata({
  ...
  return {
    title: locale === 'en' ? 'Pathways' : 'Vías',
    alternates: alternatesFor('/pathways', locale),
  };
}
...
export default async function PathwaysPage({
  ...
  return (
    <Page>
      <PageHeader
        title={t('The rule catalog', 'El catálogo de normas')}
        lead={t(
      ...
      */}
      ...
      <Section
        id="filter"
        title={t('Narrow the list', 'Acotar la lista')}
        description={t(
      ...
      >
        <Card tone="sunken">
          <form method="get" action={at('/pathways')} className={styles.filterForm}>
            <div className={styles.field}>
              <label htmlFor="jurisdiction-select" className={styles.label}>
                {t('Jurisdiction', 'Jurisdicción')}
              </label>
              <select
              ...
              >
                <option value="">{t('All jurisdictions', 'Todas las jurisdicciones')}</option>
            ...
            <div className={styles.field}>
              <label htmlFor="kind-select" className={styles.label}>
                {t('Kind of route', 'Tipo de vía')}
              </label>
              <select id="kind-select" name="kind" defaultValue={kind} className={styles.select}>
                <option value="">{t('All kinds of route', 'Todos los tipos de vía')}</option>
                {KINDS.map((value) => (
                  <option key={value} value={value}>
                    {t(pathwayKindLabel(value))}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className={styles.actions}>
              ...
              <Link href={at('/pathways')} className={styles.reset}>
                {t('Clear', 'Limpiar')}
              </Link>
            </div>
          </form>
        ...
        {filtered.length === 0 ? (
          ...
          <Stack gap="md">
            {filtered.map((pathway) => {
              ...
              const discretionary = pathway.citations.filter(
                (c) => c.discretionary === true,
              ).length;
                ...
                <article key={pathway.id} className={styles.pathway}>
                  <div className={styles.pathwayHead}>
                    <h3 className={styles.pathwayTitle}>
                      <Link href={at(`/pathways/${pathway.id}`)}>{t(pathway.name)}</Link>
                    </h3>
                  ...
                  <p className={styles.pathwaySummary}>{t(pathway.summary)}</p>

                  <ul className={styles.pathwayStats}>
                    <li>
                      {t('Criteria', 'Criterios')}
                      {': '}
                    ...
                    <li>
                      {t('Practice-based sources', 'Fuentes de práctica administrativa')}
                      {': '}
                      <strong>{discretionary}</strong>
                    </li>
                    {oldest !== null && band !== null ? (
                      <li>
                        {t('Oldest verification', 'Verificación más antigua')}
                        {': '}
                  ...
                  {pathway.closedOn !== undefined && pathway.closureNote !== undefined ? (
                    <div className={styles.closure}>
                      <strong>
                        {t('Closed to new applications', 'Cerrada a nuevas solicitudes')}
                        {' — '}
      ...
      <Section id="catalog-disclosure" title={t('What this catalog is', 'Qué es este catálogo')}>
        <DisclosureNotice
          locale={locale}
          shown="information"
          withheld={[
