...
export async function generateMetadata({
  ...
  return { title: sample === null ? 'Documents' : `Documents — ${sample.name.en}` };
}
...
function ItemCard({ item }: { readonly item: ChecklistItem }) {
      ...
      <div className={styles.itemHead}>
        <div className={styles.itemIdentity}>
          <h3 className={styles.itemTitle}>
            <T text={DOCUMENT_KIND_LABEL[requirement.kind]} />
          </h3>
        ...
        <div className={styles.itemBadges}>
          {held === null ? (
            <Badge tone="bad" label={bi('Not on file', 'No consta')} />
          ) : (
            <Badge tone="ok" label={bi('On file', 'Consta')} />
          )}
        </div>
      </div>

      <TProse text={requirement.criterion} className={styles.criterion} />

      {held !== null ? (
        <Facts>
          <Fact label={bi('Issued', 'Expedido')}>
            {held.issuedOn === undefined ? (
              <span className={styles.muted}>
                <T text={bi('Not recorded', 'Sin registrar')} />
              </span>
            ) : (
              <CivilDate value={held.issuedOn} />
            )}
          </Fact>
          <Fact label={bi('Expires', 'Caduca')}>
      ...
      <div className={styles.routing}>
        <section className={styles.routingBlock}>
          <h4 className={styles.routingTitle}>
            <T text={bi('Legalisation', 'Legalización')} />
          </h4>
          <Badge tone={LEGALISATION_TONE[legalisation.route]} label={LEGALISATION_LABEL[legalisation.route]} />
          <p className={styles.rationale}>
            <T text={legalisation.rationale} />
          </p>
          {legalisation.route !== 'none' && legalisation.route !== 'unknown' ? (
            <p className={styles.progress}>
              {item.legalisationDone ? (
                <Badge tone="ok" label={bi('Done', 'Hecha')} />
              ) : (
                <Badge tone="warn" label={bi('Outstanding', 'Pendiente')} />
              )}
            </p>
          ) : null}
        </section>
        ...
        <section className={styles.routingBlock}>
          <h4 className={styles.routingTitle}>
            <T text={bi('Translation', 'Traducción')} />
          </h4>
          <Badge
            tone={
              translation.standard === 'unknown' ? 'warn' : translation.required ? 'info' : 'ok'
            }
            label={TRANSLATION_STANDARD_LABEL[translation.standard]}
          />
          <p className={styles.rationale}>
            <T text={translation.rationale} />
          </p>
        ...
        <section className={styles.routingBlock}>
          <h4 className={styles.routingTitle}>
            <T text={bi('Currency at submission', 'Vigencia en la presentación')} />
          </h4>
          <Badge tone={FRESHNESS_TONE[freshness.verdict]} label={FRESHNESS_LABEL[freshness.verdict]} />
          <p className={styles.rationale}>
            <T text={freshness.rationale} />
          </p>
          {freshness.acceptableUntil !== null ? (
            <p className={styles.freshnessDetail}>
              <TInline text={bi('Acceptable until', 'Admisible hasta')} />{' '}
              ...
              {freshness.marginDays !== null ? (
                <span className={styles.margin}>
                  ...
                  {freshness.marginDays >= 0 ? (
                    <TInline
                      text={bi(
                        `${days(freshness.marginDays)} of margin`,
                        `${freshness.marginDays} días de margen`,
                      )}
                    />
                  ) : (
                    <TInline
                      text={bi(
                        `${days(Math.abs(freshness.marginDays))} short`,
                        `${Math.abs(freshness.marginDays)} días de retraso`,
                      )}
                    />
                  )}
                  )
          ...
          {freshness.obtainNoEarlierThan !== null ? (
            <p className={styles.freshnessDetail}>
              <TInline text={bi('Order it no earlier than', 'No lo solicite antes del')} />{' '}
...
export default async function DocumentsPage({
  ...
  return (
    <>
      <Section
        id="documents-scope"
        title={bi('What this page is', 'Qué es esta página')}
      >
        <Callout
          tone="info"
          icon="i"
        ...
        <Card tone="sunken">
          <Facts>
            <Fact label={bi('Filing with', 'Se presenta ante')}>
              <Chip>{checklist.receivingCountry}</Chip>
            </Fact>
            <Fact label={bi('Target submission date', 'Fecha objetivo de presentación')}>
            ...
            <Fact label={bi('Items required', 'Elementos exigidos')}>
              <Figure value={checklist.items.length} />
            </Fact>
            <Fact label={bi('File ready to lodge', 'Expediente listo para presentar')}>
              <Badge
                tone={checklist.complete ? 'ok' : 'warn'}
                label={
                  checklist.complete
                    ? bi('Yes', 'Sí')
                    : bi('Not yet', 'Todavía no')
                }
              />
            </Fact>
          </Facts>
      ...
      <Section
        ...
        title={bi('What needs attention', 'Qué requiere atención')}
        description={bi(
      ...
      >
        <Stack gap="md">
          <AttentionBucket
            tone="bad"
            title={bi('Not on file', 'No constan')}
            empty={bi('Every required item is on file.', 'Todos los elementos exigidos constan.')}
            items={checklist.missing}
          />
          <AttentionBucket
            tone="bad"
            title={bi('Out of date by the filing date', 'Caducados a la fecha de presentación')}
            empty={bi(
          ...
          <AttentionBucket
            tone="warn"
            title={bi('Currency never checked', 'Vigencia nunca comprobada')}
            empty={bi(
          ...
          <AttentionBucket
            tone="warn"
            title={bi('Routing a person must confirm', 'Enrutado que debe confirmar una persona')}
            empty={bi(
          ...
          <AttentionBucket
            tone="info"
            title={bi('Legalisation outstanding', 'Legalización pendiente')}
            empty={bi(
          ...
          <AttentionBucket
            tone="info"
            title={bi('Translation outstanding', 'Traducción pendiente')}
            empty={bi(
      ...
      <Section
        ...
        title={bi('The checklist', 'La lista de documentos')}
        description={bi(
      ...
      >
        <Stack gap="md">
          {checklist.items.map((item) => (
            <ItemCard key={item.requirement.key} item={item} />
          ))}
        </Stack>
      </Section>
      ...
      <Section
        ...
        title={bi('What these requirements are', 'Qué son estos requisitos')}
      >
        <DisclosureNotice
          shown="assessment"
          withheld={[
      ...
      <ul className={styles.bucketList}>
        {items.map((item) => (
          <li key={item.requirement.key}>
            <TInline text={DOCUMENT_KIND_LABEL[item.requirement.kind]} />
