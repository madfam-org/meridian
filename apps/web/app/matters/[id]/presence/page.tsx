...
export async function generateMetadata({
  ...
  return { title: sample === null ? 'Day counters' : `Day counters — ${sample.name.en}` };
}
...
function rangeRows(ranges: readonly DateRange[]): WorkingRow[] {
  return ranges.map((r) => ({
    label: bi(`${r.start} to ${r.end}`, `${r.start} a ${r.end}`),
    op: '+',
    value: days(rangeLengthDays(r)),
  }));
}

/** The caveat a discretionary source obliges the page to show in place of the number. */
function DiscretionaryCaveat({ citation }: { readonly citation: Citation }) {
...
export default async function PresencePage({
  ...
  return (
    <>
      <Section
        id="record"
        title={bi('The record these counts rest on', 'El registro en el que se basan los cómputos')}
        description={bi(
      ...
      >
            ...
            <tbody>
              {report.ledger.stays.map((stay) => (
                <tr key={stay.id}>
                  ...
                  <td className={styles.small}>{stay.source.replace(/_/g, ' ')}</td>
                  <td className={styles.small}>{stay.confidence}</td>
                  <td className={styles.small}>
                    <div className={styles.noteStack}>
                      ...
                      {stay.exemptFromSchengenShortStay === true ? (
                        <Badge
                          tone="info"
                          label={bi('Not a short stay', 'No es estancia corta')}
                        />
                      ) : null}
                    </div>
                  </td>
        ...
        <div>
          <h3 className={styles.subheading}>
            <T text={bi('Record quality', 'Calidad del registro')} />
          </h3>
              ...
              {report.inconsistencies.map((issue, index) => (
                <li key={`${issue.kind}-${issue.range.start}-${index}`} className={styles.issue}>
                  <div className={styles.issueHead}>
                    <Badge {...inconsistencyView(issue.kind)} />
                    <RangeText range={issue.range} />
                    <span className={styles.numeric}>{days(issue.days)}</span>
                  </div>
      ...
      {report.schengen !== null ? (
        <Section
          id="schengen"
          title={bi('Schengen short-stay allowance', 'Franquicia Schengen de estancia corta')}
          description={scope.schengenBasis}
          ...
          <Card>
            <div className={styles.headline}>
              <Figure
                ...
                unit={bi(`of ${SCHENGEN_MAX_DAYS} days used`, `de ${SCHENGEN_MAX_DAYS} días usados`)}
                tone="strong"
              />
              <Badge
            ...
            <Facts>
              <Fact label={bi('Window assessed', 'Ventana evaluada')}>
                <RangeText
                  range={{
              ...
              <Fact label={bi('Days still available', 'Días aún disponibles')}>
                <Figure
                  value={report.schengen.value.daysRemaining}
                  unit={bi('days', 'días')}
                />
              </Fact>
              <Fact label={bi('Days over the limit', 'Días por encima del límite')}>
                <Figure value={report.schengen.value.daysOverLimit} unit={bi('days', 'días')} />
              </Fact>
            </Facts>
            ...
            <Working
              rows={[
                ...
                ...rangeRows(report.schengen.value.countedRanges),
                {
                  label: bi(
                    'Days charged against the allowance',
                    'Días imputados a la franquicia',
                  ),
                  op: '=',
                  value: days(report.schengen.value.daysUsed),
                  note: bi(
              ...
              <ScrollX>
                <table>
                  ...
                  <thead>
                    <tr>
                      <th scope="col">Country</th>
                      <th scope="col">Counted period</th>
                      <th scope="col">Days</th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        <Section
          ...
          title={bi('Schengen short-stay allowance', 'Franquicia Schengen de estancia corta')}
        >
          <Empty text={scope.schengenBasis} />
        </Section>
      )}

      <Section
        id="tax-day-counts"
        title={bi('Tax residence day counts', 'Cómputo de días para residencia fiscal')}
        description={bi(
          ...
          {report.thresholds.map(({ threshold, assessment }) => {
            ...
            return (
              <Card key={threshold.id}>
                <div className={styles.headline}>
                  <div>
                    ...
                    <p className={styles.thresholdMeta}>
                      <Chip>{threshold.country}</Chip>{' '}
                      <CitationRefs ids={assessment.citationIds} />
                    </p>
                  </div>
                ...
                <Facts>
                  <Fact label={bi('Days recorded', 'Días registrados')}>
                    <Figure value={value.daysPresent} unit={bi('days', 'días')} tone="strong" />
                  </Fact>
                  <Fact label={bi('Days needed', 'Días necesarios')}>
                    <Figure value={value.requiredDays} unit={bi('days', 'días')} />
                  </Fact>
                  <Fact label={bi('Margin', 'Margen')}>
                    <Figure value={Math.abs(value.marginDays)} unit={bi('days', 'días')} />
                    <div className={styles.factNote}>
                      <T
                        text={
                          value.met
                  ...
                  <Fact label={bi('Window counted', 'Ventana computada')}>
                  ...
                  rows={[
                    {
                      ...
                      value: days(threshold.thresholdDays),
                      note:
                    ...
                    },
                    {
                      ...
                      op: '=',
                      value: days(value.requiredDays),
                    },
                    ...rangeRows(value.countedRanges),
                    {
                      label: bi('Days present in the window', 'Días presentes en la ventana'),
                      op: '=',
                      value: days(value.daysPresent),
                    },
                    {
                      label: value.met
                        ? bi('Days past the threshold', 'Días por encima del umbral')
                ...
                <Facts>
                  <Fact label={bi('Reached on', 'Alcanzado el')}>
                    {value.metOn === null ? (
                      <span className={styles.muted}>
                        <T text={bi('Not reached', 'No alcanzado')} />
                      </span>
                    ) : (
                      <CivilDate value={value.metOn} />
                    )}
                  </Fact>
                  <Fact label={bi('Would be reached on', 'Se alcanzaría el')}>
                ...
                <DiscretionaryCaveat citation={threshold.citation} />
              </Card>
            );
          })}
        </Stack>
      ...
      <Section
        id="continuity"
        title={bi('Continuity of residence', 'Continuidad de la residencia')}
        description={
          scope.continuity?.basis ??
          bi('No continuity policy applies to this matter.', 'A este expediente no le aplica ninguna política de continuidad.')
        }
      >
        {report.continuity === null ? (
          ...
          <Card>
            ...
            <DiscretionaryCaveat citation={report.continuity.value.policy.citation} />

            <Facts>
              <Fact label={bi('Period assessed', 'Periodo evaluado')}>
                <RangeText range={report.continuity.value.window} />
              </Fact>
              <Fact label={bi('Days in country', 'Días en el país')}>
                <Figure value={report.continuity.value.presenceDays} unit={bi('days', 'días')} />
              </Fact>
              <Fact label={bi('Days absent', 'Días de ausencia')}>
                <Figure value={report.continuity.value.totalAbsenceDays} unit={bi('days', 'días')} />
              </Fact>
              <Fact label={bi('Longest single absence', 'Ausencia continua más larga')}>
                <Figure
            ...
            <div>
              <h4 className={styles.subheading}>
                <T text={bi('What the policy limits', 'Qué limita la política')} />
              </h4>
              <ul className={styles.limbList}>
                <li>
                  <TInline text={bi('Longest single absence', 'Ausencia continua más larga')} />
                  {': '}
                  {report.continuity.value.policy.maxSingleAbsenceDays === undefined ? (
                    <span className={styles.muted}>
                      <TInline text={bi('no limit encoded', 'sin límite codificado')} />
                    </span>
                  ) : (
                    <strong className={styles.numeric}>
                      {days(report.continuity.value.policy.maxSingleAbsenceDays)}
                    </strong>
                  )}
                </li>
                <li>
                  <TInline text={bi('Absence within one residence year', 'Ausencia dentro de un año de residencia')} />
                  {': '}
                  {report.continuity.value.policy.maxCumulativeAbsenceDaysPerYear === undefined ? (
                    ...
                    <strong className={styles.numeric}>
                      {days(report.continuity.value.policy.maxCumulativeAbsenceDaysPerYear)}
                    </strong>
                  )}
                </li>
                <li>
                  <TInline text={bi('Absence across the whole period', 'Ausencia en todo el periodo')} />
                  {': '}
                  {report.continuity.value.policy.maxCumulativeAbsenceDaysTotal === undefined ? (
                    ...
                    <strong className={styles.numeric}>
                      {days(report.continuity.value.policy.maxCumulativeAbsenceDaysTotal)}
                    </strong>
                  )}
                </li>
              </ul>
            ...
            {report.continuity.value.breaches.length > 0 ? (
              <div>
                <h4 className={styles.subheading}>
                  <T text={bi('Where the criterion is exceeded', 'Dónde se supera el criterio')} />
                </h4>
                <ul className={styles.issueList}>
                  {report.continuity.value.breaches.map((breach, index) => (
                    <li key={`${breach.limb}-${index}`} className={styles.issue}>
                      <div className={styles.issueHead}>
                        <Badge tone="warn" label={continuityLimbLabel(breach.limb)} />
                        <RangeText range={breach.range} />
                        <span className={styles.numeric}>
                          {days(breach.days)} / {days(breach.limitDays)}
                        </span>
                      </div>
                      <p className={styles.issueDetail} lang="en">
                        {breach.detail}
            ...
            <div>
              <h4 className={styles.subheading}>
                <T text={bi('Residence years', 'Años de residencia')} />
              </h4>
              <p className={styles.footnote}>
                <T
                  text={bi(
            ...
            <div>
              <h4 className={styles.subheading}>
                <T text={bi('Absences counted', 'Ausencias computadas')} />
              </h4>
              {report.continuity.value.absences.length === 0 ? (
                ...
                <Working
                  ...
                  rows={[
                    ...rangeRows(report.continuity.value.absences),
                    {
                      label: bi('Total days absent', 'Total de días de ausencia'),
                      op: '=',
                      value: days(report.continuity.value.totalAbsenceDays),
                      emphasis: true,
                    },
                  ]}
                />
      ...
      <Section id="presence-disclosure" title={bi('What these figures are', 'Qué son estas cifras')}>
        <DisclosureNotice
          shown="assessment"
          withheld={[
            bi(
