 ...
 */
export function CoverageBoundary({ jurisdictions, locale }: CoverageScope) {
  ...
  return (
    <section className={styles.boundary} id={COVERAGE_BOUNDARY_ID} aria-labelledby={headingId}>
      ...
      <p className={styles.lead}>{t(NOT_A_VERDICT_ON_YOU)}</p>

      <div className={styles.covered}>
        <h3 className={styles.subheading}>{t('What is encoded', 'Qué está codificado')}</h3>
            ...
            return (
              <li key={code}>
                <p className={styles.jurisdictionHead}>
                  <Chip>{code}</Chip> {t(jurisdictionName(code))}
                  {' — '}
                  <strong>
                    {t(
                ...
                {routes.length === 0 ? null : (
                  <ul className={styles.routes}>
                    {routes.map((route) => (
                      <li key={route.id}>
                        <Link href={`/pathways/${route.id}`}>{t(route.name)}</Link>
                      </li>
                    ))}
                  </ul>
                )}
      ...
      <div className={styles.missing}>
        <h3 className={styles.subheading}>{t('What is not encoded', 'Qué no está codificado')}</h3>
          ...
          <ul className={styles.gaps}>
            {missing.map((route) => (
              <li className={styles.gap} key={route.key}>
                ...
                <p className={styles.gapSource}>{t(route.source)}</p>
              </li>
            ))}
          </ul>
        )}
        ...
        <p className={styles.footnote}>{t(REGISTER_IS_NOT_EXHAUSTIVE)}</p>
      </div>

      <div className={styles.block}>
        <h3 className={styles.subheading}>
          {t('Deliberately out of scope', 'Deliberadamente fuera de alcance')}
        </h3>
        <p>{t(OUT_OF_SCOPE_PROTECTION)}</p>
      </div>

      <div className={styles.block}>
        <h3 className={styles.subheading}>
          {t('Who to ask instead', 'A quién preguntar en su lugar')}
        </h3>
        <p>{t(WHERE_TO_ASK)}</p>
      </div>
    </section>
 ...
 */
export function CoverageResultNotice({ jurisdictions, locale }: CoverageScope) {
  const t = translator(locale);
  const scope = scopeOf(jurisdictions);
  const missing = uncoveredIn(scope);
  ...
  return (
    <aside className={styles.notice} role="note">
      ...
      <p>{t(NOT_A_VERDICT_ON_YOU)}</p>

      {missing.length > 0 ? (
        <>
          <p className={styles.noticeLabel}>
            {t('Not encoded anywhere in Meridian', 'No codificado en ninguna parte de Meridian')}
          </p>
