...
export function MrzTool({ locale }: { readonly locale: Locale }) {
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>): void {
    ...
    const dateResult = readDateField(REFERENCE_ID, reference, {
      required: true,
    });
    const found = collect([
      requireText(
  ...
  return (
    <Stack gap="lg">
      ...
      <Section
        id="mrz-input"
        title={t('The lines from the document', 'Las líneas del documento')}
        description={t(
        ...
        <Card tone="sunken">
          ...
          <form className={styles.form} onSubmit={onSubmit} noValidate>
            <TextAreaField
              locale={locale}
              id={LINES_ID}
              label={t('Machine-readable lines', 'Líneas de lectura mecánica')}
              hint={t(
              ...
              error={t(issueFor(LINES_ID, issues))}
              required
              mono
              verbatim
              rows={4}
            ...
            <DateField
              ...
              id={REFERENCE_ID}
              label={t(
                'Resolve two-digit years as at',
                'Resolver los años de dos dígitos a fecha de',
              )}
              hint={t(
              ...
              error={t(issueFor(REFERENCE_ID, issues))}
              required
              value={reference}
            ...
            <ToolActions>
              <ToolButton
                type="submit"
                variant="primary"
                label={t('Check the zone', 'Comprobar la zona')}
            ...
            <ToolActionGroup
              id="mrz-specimens"
              label={t('Or load a synthetic specimen', 'O cargue un ejemplar sintético')}
            >
      ...
      {report !== null ? (
        <MrzResult report={report} focusKey={resultFocusKey} locale={locale} />
      ) : null}
    </Stack>
  );
}
...
function MrzResult({
  ...
  return (
    <ResultPanel
      id={RESULT_ID}
      title={t('What the zone says', 'Qué dice la zona')}
      verdict={t(report.selfConsistent ? VERDICT_SOUND : VERDICT_BROKEN)}
      tone={report.selfConsistent ? 'ok' : 'bad'}
      lead={t(report.selfConsistent ? VERDICT_LEAD_SOUND : VERDICT_LEAD_BROKEN)}
      focusKey={focusKey}
    >
      <ResultBlock
        id="mrz-format"
        title={t('Format', 'Formato')}
        description={t(
      ...
      >
        {report.formatName === null ? (
          ...
          <p className={styles.formatLine}>
            <PlainBadge tone="info">{report.format}</PlainBadge> {t(report.formatName)}
          </p>
        )}
      ...
      {report.fields.length > 0 ? (
        <ResultBlock
          id="mrz-fields"
          title={t('Every field it could read', 'Todos los campos que pudo leer')}
          description={t(
        ...
        >
          <ScrollX>
            <table>
              <thead>
                <tr>
                  <th scope="col">{t('Field', 'Campo')}</th>
                  <th scope="col">{t('Reading', 'Lectura')}</th>
                  <th scope="col">{t('As printed', 'Tal como está impreso')}</th>
                </tr>
              </thead>
              <tbody>
                {report.fields.map((field) => (
                  <tr key={field.key}>
                    ...
                    <td>
                      {field.value === '' ? (
                        <span className={styles.absent}>{t(NOT_RECORDED)}</span>
                      ) : (
                      ...
                      {field.note !== undefined ? (
                        <p className={styles.fieldNote}>{t(field.note)}</p>
                      ) : null}
                    </td>
                    <td>
      ...
      {report.checks.length > 0 ? (
        <ResultBlock
          id="mrz-check-digits"
          title={t('Each check digit, separately', 'Cada dígito de control, por separado')}
          description={t(
        ...
        >
          <ScrollX>
            <table>
              <thead>
                <tr>
                  <th scope="col">{t('Digit for', 'Dígito de')}</th>
                  <th scope="col">{t('At', 'En')}</th>
                  <th scope="col">{t('In the document', 'En el documento')}</th>
                  <th scope="col">{t('Computed', 'Calculado')}</th>
                  <th scope="col">{t('Verdict', 'Veredicto')}</th>
                </tr>
              </thead>
              <tbody>
                {report.checks.map((check) => (
                  <tr key={check.key}>
                      ...
                      {check.covers !== '' ? (
                        <p className={styles.covers}>
                          <span className={styles.coversLabel}>
                            {t('Computed over', 'Calculado sobre')}
                          </span>{' '}
                    ...
                    <td className={styles.nowrap}>
                      {t('line', 'línea')} {check.line}, {t('column', 'columna')} {check.column}
                    </td>
                    <td>
                    ...
                    <td>
                      {check.expected === null ? (
                        <span className={styles.absent}>
                          {t('Filler permitted', 'Se admite relleno')}
                        </span>
                      ) : (
      ...
      {report.failures.length > 0 ? (
        <ResultBlock
          id="mrz-failures"
          title={t('What does not add up', 'Qué no cuadra')}
          description={t(
            ...
            {report.failures.map((failure) => (
              <li className={styles.failure} key={failure.key}>
                <div className={styles.failureHead}>
                  <Badge tone="bad" label={t(failure.code)} />
                  {failure.line !== undefined && failure.column !== undefined ? (
                    <Chip>
                      {t('line', 'línea')} {failure.line}, {t('column', 'columna')} {failure.column}
                    </Chip>
                  ) : null}
                </div>
      ...
      {report.resolvedADate ? (
        ...
        >
          <p>{t(CENTURY_WINDOW_NOTE)}</p>
          <p className={styles.referenceLine}>
            {t(
              'Two-digit years were resolved as at',
              'Los años de dos dígitos se resolvieron a fecha de',
            )}{' '}
      ...
      <ResultBlock id="mrz-sources" title={t('The rule this applied', 'La norma aplicada')}>
        <CitationList
          locale={locale}
          citations={[ICAO_9303_CITATION]}
          asOf={report.referenceDate}
        />
      </ResultBlock>
