 ...
 * function, and it sits in the path of EVERY header this site sends — so the
...
describe('the deployed entry', () => {
  ...
  it('announces itself on boot', () => {
    ...
    expect(ENTRY_SOURCE).toContain('meridian-landing: internal response header filter active');
  });
...
describe('the entry and this test agree with the other applications', () => {
  it('declares the same list web and admin declare', () => {
